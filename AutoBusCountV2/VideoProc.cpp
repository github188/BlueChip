...
CVideoProc::CVideoProc(void)
{
	...
	m_height=100; //m4
	m_rheight=10; 
	m_y=288-(int)m_height*0.5; //m3
	rect=cvRect(m_x,m_y,m_width,m_height); //检测区大小
	rect1=cvRect(0,0,m_width,m_rheight);
...
///////////////////////
	...
	m_iManMaxHeight=400; //m5
	m_iRoiRealTopPos=0; //m6
	m_iRoiRealHeight=576; //m7
}
...
int CVideoProc::GetVar(const int m1,const int m2, const int m3,const int m4,const int m5,const int m6,const int m7)
{
	m_iLineIn=m1;
	m_iLineOut=m2;
	m_y=m3;
...
{	
	...
	if((destination[0].y+destination[0].height > 2*m_iManMaxHeight )&&(destination[0].height>m_iManMaxHeight))
	{
		//DrawRect(absthresh,destination[0],CV_RGB(255,255,255));
		//DrawRect(image,destination[0],CV_RGB(0,255,0));
	}
	if((p>1)&&(destination[1].height>m_iManMaxHeight)) //说明两个人连续上车
	{
	...
	if(((float)destination[0].y+(destination[0].height*0.5)-m_iLineIn)*((float)predestination[0].y+(predestination[0].height*0.5)-m_iLineIn)<0.00f)	
	{
		if(((float)destination[0].y+(destination[0].height*0.5)-m_iLineIn)<0.00f)
		{
			...
			if(m_bIn)
			{
				...
				m_bIn=false;
				Beep(440,250);
				if(destination[0].height>m_iManMaxHeight)
				{
					m_iIn++;
					Beep(440,250);
				}
