...
int CUsbStream::set_save(int save_fps,char* dir){
        cout<<"begin setting save files..."<<endl;
        cout<<"save dir:"<<dir<<endl;
        saveFps = save_fps;
        init_dir = dir;
        bSave = true;
        string cmd = "mkdir -p i";
        string _tmp_dir = init_dir;
        cmd += _tmp_dir;
        ...
        pclose(pp);
        time_t t = time(0);
        strftime(filename,sizeof(filename),"%Y%m%d%H.h264",localtime(&t));
        cout<<"filename:"<<filename<<endl;
        string _tmp_filename = init_dir;
        char* _tmp = filename;
        _tmp_filename += _tmp;
        strcpy(filename,_tmp_filename.data());
