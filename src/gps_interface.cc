...
double IConGPS::get_latitude(){
	return GPS->latitude;
}

double IConGPS::get_longitude(){
...
double IConGPS::get_double_number(char* s){
	char buf[128];
	int i;
	double rev;
	i=get_comma(1,s);
...
double IConGPS::get_locate(double temp){
	int m;
	double n;
	m=(int)temp/100;
	n=(temp-m*100)/60;
...
int IConGPS::get_comma(int num,char* str)
{
	int i,j=0;
	int len=strlen(str);
	for(i=0;i<len;i++){
...
void IConGPS::UTC2BTC(date_time* GPS){
	GPS->second++;
	if(GPS->second>59){
		GPS->second=0;
		GPS->minute++;
...
void IConGPS::readGpsDev(){
	receive();
	gps_parse();
}

int IConGPS::set_opt(int fd,int nSpeed,int nBits,char nEvent,int nStop){
...
int IConGPS::closeGpsDev(){
	close(Gpsfd);
	return 0;
}
void IConGPS::gps_parse(){
	int i=0;
	int j=0;
	char buf[1024];
	while(GPS_BUF[i]!='\0'){
